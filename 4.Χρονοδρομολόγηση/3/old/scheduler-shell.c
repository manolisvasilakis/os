...
sched_set_high_by_id(int id)
{
	...
	struct Process_List *temp = head->next, *prev = head;

	while (temp != NULL) {
		if (temp->id == id) {
			if (temp->priority == 1) return;	//no change
...
sched_set_low_by_id(int id)
{
	...
	if (head->id == id) {
		if (head->priority == 0) return;	//no change

		head->priority = 0;
		if (head->next == NULL || head->next->priority == 0) {
			last_high = NULL;
			return;
		}
	...
	while (temp != NULL) {
		if (temp->id == id) {
			if (temp->priority == 0) return;	//no change

			temp->priority = 0;

			if (prev->priority == 1 && (temp->next == NULL || temp->next->priority == 0)) last_high = prev;
			else {
...
{
	int change_current = 0;
	if (current->pid == p) change_current = 1;
	...
	if (head->pid == p) {
		...
		free (head);
		head = temp;
		if (head->priority == 0) last_high = NULL;
		if (change_current) {
			current = head;
			activate_current ();
		}
	...
	while (temp != NULL) {
		if (temp->pid == p) {
			...
			free (temp);
			if (change_current) {
				if (prev->next == NULL) current = head;
