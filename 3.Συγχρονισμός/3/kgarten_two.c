...
struct kgarten_struct {
	...
	pthread_cond_t cond_child, cond_teach;
...
void child_enter(struct thread_info_struct *thr)
{
	...
	while((thr->kg->vt)*(thr->kg->ratio) == thr->kg->vc) {			//oso o max arithmos paidion == arithmos paidion, kane wait
		ret = pthread_cond_wait(&thr->kg->cond_child, &thr->kg->mutex);
		if (ret) {
			perror_pthread(ret, "cond_child_wait");
			exit(1);
		}
	}
...
void child_exit(struct thread_info_struct *thr)
{
	...
	/* Give priority to teachers, if they can leave, they leave */
	if (thr->kg->vc <= (thr->kg->vt - 1) * thr->kg->ratio) {			//an arithmos paidion <= ratio*(teacher - 1) tote na fygei teacher
		ret = pthread_cond_signal(&thr->kg->cond_teach);				//bazoume signal, oxi broadcast gia na apofygoume askopa jypnhmata, edo isos broadcast
		if (ret) {
			perror_pthread(ret, "cond_teach_signal");
			exit(1);
		}
	} else {
		ret = pthread_cond_signal(&thr->kg->cond_child);				//bazoume signal, oxi broadcast gia na apofygoume askopa jypnhmata
		if (ret) {
			perror_pthread(ret, "cond_child_signal");
			exit(1);
		}
	}
...
void teacher_enter(struct thread_info_struct *thr)
{
	...
	ret = pthread_cond_broadcast(&thr->kg->cond_child);						// ratio clildren may enter, ara broadcast
	if (ret) {
		perror_pthread(ret, "cond_child_broadcast");
		exit(1);
	}
...
void teacher_exit(struct thread_info_struct *thr)
{
	...
	while(thr->kg->vc > (thr->kg->vt - 1) * thr->kg->ratio) {				//oso paidia > max paidia gia -1 daskalous
		ret = pthread_cond_wait(&thr->kg->cond_teach, &thr->kg->mutex);
		if (ret) {
			perror_pthread(ret, "cond_teach_wait");
			exit(1);
		}
	}
...
int main(int argc, char *argv[])
{
	...
	ret = pthread_cond_init(&kg->cond_teach, NULL);
	if (ret) {
		perror_pthread(ret, "pthread_cond_init");
		exit(1);
	}
	...
	/* Destroy cond & mutex */
	ret = pthread_cond_destroy(&kg->cond_teach);
	if (ret) {
		perror_pthread(ret, "pthread_cond_destroy");
		exit(1);
	}
	ret = pthread_cond_destroy(&kg->cond_child);
	if (ret) {
		perror_pthread(ret, "pthread_cond_destroy");
		exit(1);
	}
